    ...
    <div class="card card-secondary card-outline">
        <div class="card-body sinpadding">
            <div class="col-12 mb-2">
                ...
                <button id="file_trabajador" class="btn btn-primary btn-sm">File Digital <i
            ...
            <x-datatable id="table_trabajador"
                        :heads="[
                            'Nro',
                            'DOCUMENTO(CI)',
                            'NRO. ASEGURADO',
    ...
    <script>
        function lista_trabajadores() {
            ...
            table_trabajador = $('#table_trabajador').DataTable({
                ...
                columns: [
                    ...
                    {
                        render: function(data, type, row) {
                            trabajador = row['antiguedad_anios'] + ' AÑOS <br> ' + row['antiguedad_meses'] + ' MESES<br> ' + row[
                    ...
                    },
                    {
                        render: function(data, type, row) {
                            if (row['estado_trabajador'] == 'HABILITADO') {
                                estado_t = "<span class='badge badge-success' >" + row[
                                    'estado_trabajador'] + "</span>"
                                return estado_t;
                            } else {
                                estado_t = "<span class='badge badge-danger' >" + row['estado_trabajador'] +
                                    "</span>"
                                return estado_t;
                            }
                        }
                ...
                buttons: [{
                    ...
                    },
                    {
                        extend: 'print',
                        footer: true,
                        text: '<i class="fas fa-print" data-toggle="tooltip" title="Imprimir todas las paginas"></i>',
                        className: 'btn btn-sm btn-danger',
                        exportOptions: {
                            modifier: {
                                order: 'applied',
                    ...
                    },
                    {
                        extend: 'excelHtml5',
                        footer: true,
                        text: '<i class="fas fa-file-excel" data-toggle="tooltip" title="Hoja de Excel Todas las páginas"></i>',
                        className: 'btn btn-sm btn-success',
                        exportOptions: {
                            modifier: {
                                order: 'applied',
        ...
        $(() => {
            ...
            $('.filter-select').change(function() {
                if ($(this).val() == 'TODOS') {
                    var table_trabajador = $('#table_trabajador').DataTable();
                    table_trabajador
                        .search('')
                        .columns().search('')
                        .draw();
            ...
            $('#modificar_trabajador').on('click', function() {
                ...
                if (data.length != 0) {
                    var rowId = data[0]['id'];
                    var urlEditTrabajador = "/trabajador/" + rowId + "/edit";
