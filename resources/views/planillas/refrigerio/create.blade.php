        ...
        <div class="col-md-7">
            <div class="card card-primary ">
                <div class="card-header">
                    <h3 class="card-title">Crear asistencia individual</h3>
                </div>
                <form action="{{ route('asistencia.generar_asistencia') }}" method="POST" class="create" role="form"
                    id="form_all_asistencia">
                    @csrf
                    <div class="card-body row">
                        ...
                        {{-- Trabajadores que no tenga asistencia --}}
                        ...
                        <x-input type="number" name="dias_asistencia" id="dias_asistencia" label="Días Asistencia" min="1"
                            topclass="col-md-12 required" value="{{ old('dias_asistencia', 30) }}" />
                        {{-- Componente de Dias de asistencia --}}
                        <x-input type="number" name="dias_laborales" id="dias_laborales" label="Días Laborales" min="1"
                            topclass="col-md-12 required" value="{{ old('dias_laborales', 30) }}" />
                        <input type="hidden" name="tipo_contrato" id="tipo_contrato" value="{{ $tipo_contrato }}">
                    </div>
