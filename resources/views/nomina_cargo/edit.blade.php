    ...
    <div class="justify-content-center row">
        <div class="col-md-7">
            <div class="card card-info ">
                ...
                <form action="{{ route('nomina_cargo.update',$nomina_cargo) }}" method="POST" class="create" role="form"
                    id="form_nomina_cargo">
                    @method('PUT')
                    @csrf
                    <div class="card-body row">
                        <x-dg-select2 id="tipo_contrato_id" name="tipo_contrato_id" label="Tipo de Contrato"
                            topclass="col-md-6 tipo_contrato_id required">
                            <option value="">--SELECCIONE--</option>
                            @foreach ($tipo_contratos as $t_contrato)
                                <option value="{{ $t_contrato->id }}" {{$nomina_cargo->tipo_contrato_id==$t_contrato->id ? 'selected':''}}>
                        ...
                        <div id="content_item"  class="form-group col-md-6 required d-none">
                            ...
                            <input type="number" class="form-control" name="item" id="item" min="0" value="{{ old('item', $nomina_cargo->item) }}">
                        ...
                        <x-input-icon-btn type="text" id="unidad_organizacional_nombre" idbutton="buscar_unidad_o" label="Unidad Organizacional"
                            topclass="col-md-12 required" disabled="true" icon="fas fa-search" value="{{$nomina_cargo->unidad_organizacional->seccion}}" />
                        <input type="hidden" id="unidad_organizacional_id" name="unidad_organizacional_id" value="{{$nomina_cargo->unidad_organizacional_id}}">

                        <x-input-icon-btn type="text" id="cargo_nombre" idbutton="buscar_cargo" label="Cargo" topclass="col-md-12 required"
                            disabled="true" icon="fas fa-search" value="{{$nomina_cargo->cargo->nombre}}" />
                        <input type="hidden" id="cargo_id" name="cargo_id" value="{{$nomina_cargo->cargo_id}}">
                        ...
                        <x-input-icon-btn type="text" id="escala_salarial_nivel" idbutton="buscar_escala_salarial" label="Escala Salarial" topclass="col-md-12 required"
                            disabled="true" icon="fas fa-search" value="{{'Nivel '.$nomina_cargo->escala_salarial->nivel.' - Bs.'. $nomina_cargo->escala_salarial->salario_mensual}}"/>
                        <input type="hidden" id="escala_salarial_id" name="escala_salarial_id" value="{{$nomina_cargo->escala_salarial_id}}">
...
@section('js')
    <script>
        $(() => {
            var valor = $('#tipo_contrato_id').val();
            if(valor == 1){
